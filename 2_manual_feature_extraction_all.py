...
def extract_manual_features(bin_metadata, segment_metadata):
    ...
    for _, row in tqdm(bin_metadata.iterrows(), total=len(bin_metadata), desc="Extracting Manual Features"):
        ...
        for segment in segments:
            psd_feats = extract_psd_features(segment)
            time_feats = extract_time_domain_features(segment)
...
def main():
    ...
    X.to_pickle("features_all/manual_features_all.pkl")
    y.to_csv("features_all/manual_labels_all.csv", index=False)
