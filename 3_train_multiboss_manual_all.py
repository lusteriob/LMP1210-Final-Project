"""
THIS IS THE CODE WE USED IN THE PRESENTATION!
THIS IS THE CORRECT FEATURE EXTRACTION CODE FOR THE MANUAL, MULTIBOSS AND COMBINED FEATURES.
"""
import os
import numpy as np
import pandas as pd
...
def load_feature_matrix(name):
    if name == "multiboss":
        df = pd.read_pickle("features_all/multiboss_features.pkl")
    else:
        df = pd.read_pickle(f"features_all/{name}_features_pads_style.pkl")
...
y = pd.read_csv("features_all/manual_labels_pads_style.csv")["Label"]
...
# === Output dir ===
model_dir = "models/svm_nested_cv_multiboss_all"
